...
void gfxcoder::sfft5x5_r2c_grid_perm()
{
    uint2 c[24];
    ...
    s_sync(lgkmcnt(0));
    //==================================================================
    //d_r+=gidy*ldy+imap*ldx+icell_y*tile_y*nx+icell_x*tile_x+x
    //==================================================================
    s_shr_b32(grid_y,grid,16);
    s_shr_b32(tile_y,tile,16);
    s_and_b32(grid_x,grid,s20);
    s_and_b32(tile_x,tile,s20);
    CALC_SPLIT_IDX(imap,icell_y,icell,grid_x,xmagic,ymagic,s22);
    s_mul_i32(s10,tile_y,nx);
    v_mul_lo_u32(v42,ldx,imap);
    v_mad_u24(ox,tile_x,icell_x,x);
    v_mad_u24(oy,s10,icell_y,v42);
    v_add_shl_u32(rvo,ox,oy,2);
    S_MUL_U64_U32(s8,s21,ldy);
    S_ADD_U64(rbase,rbase,s8);
    //==================================================================
    FFT32x32_R2C_LDR_SPLIT(c,rbase,rvo,ox,icell,ncells,nx);
