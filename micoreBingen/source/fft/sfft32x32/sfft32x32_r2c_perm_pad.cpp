...
/*args : d_c, ldc, bks, d_r, ncells, pad_x, ldx, ldy, nx, ny*/

void gfxcoder::sfft5x5_r2c_perm_pad( uint32_t pad_y )
{
    uint2 c[24];
    ...
    const uint32_t x    =v34;
    const uint32_t y    =v35;
    const uint32_t v    =v36;
    const uint32_t px   =v37;
    ...
    s_sync(lgkmcnt(0));
    //===========================================================
    //d_r+=by*ldy+icell*ldx+x-pad_x
    //===========================================================
    s_shr_b32(s12,pad_x,31);
    s_and_b32(pad_x,ic(31),pad_x);
    s_mov_b32(s13,gidx);
    s_cmpk_u32(s12,0,eq);
    s_cmov_b32(bx,gidy);
    s_csel_b32(by,s13,gidy);
    v_shl_or_b32(icell,bx,4,y);
    v_mul_lo_u32(v38,ldx,icell);
    v_sub_i32(px,x,pad_x);
    s_shl_b32(s14,by,2);
    S_MUL_U64_U32(s12,s14,ldy);
    S_ADD_U64(rbase,rbase,s12);
    v_add_shl_u32(rvo,v38,px,2);
    FFT32x32_R2C_LDR_PAD(c,rbase,rvo,px,icell,ncells,nx,ny,pad_y);
    ...
    //===========================================================
    //d_c+=by*ldc+(bx<<4)+q*bks+p
    //===========================================================
    v_shl_or_b32(v36,bx,4,p);
    s_shl_b32(s4,bx,7);
    s_shl_b32(s5,by,3);
