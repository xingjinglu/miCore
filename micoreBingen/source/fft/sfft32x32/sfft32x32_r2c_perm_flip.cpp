...
/*args : d_c, ldc, bks, d_r, ldx, ldy, nx, ncells*/

void gfxcoder::sfft5x5_r2c_perm_flip( uint32_t ny )
{
    uint2 c[24];
    ...
    s_shl_b32(s12,gidy,2);
    s_sync(lgkmcnt(0));
    //===========================================================
    //d_r+=by*ldy+icell*ldx+nx*ny-x-1
    //===========================================================
    s_mul_i32(s13,nx,ic(ny));
    S_MUL_U64_U32(s10,s12,ldy);
    S_ADD_U64(rbase,rbase,s10);
    v_mul_lo_u32(v37,ldx,icell);
    v_add_u32(v39,ic(1),x);
    v_sub_u32(v38,s13,v39);
    v_add_shl_u32(rvo,v37,v38,2);
    FFT32x32_R2C_LDR(c,rbase,rvo,x,icell,ncells,nx,ny);
