...
void gfxcoder::sfft5x5_c2r_g1x9()
{
    uint2 c[24];
    ...
    //m=pnc*9-gidx*72
    //n=gidx<nb-1?72:m
    //if(flipx<9){ smem[y*9+flipx]=c[i] } sync
    ...
    s_cmov_b32(s8,s9);
    v_mul_u24(v32,ic(36),y);
    v_cmp_u32(zero,flipx,eq);
    s_barrier();
    s_andexec_b64(s0,vcc);
    lsst_b32(v32,0x00,c[ 0].x);
    lsst_b32(v32,0x04,c[ 0].y);
    lsst_b32(v32,0x08,c[ 8].x);
    lsst_b32(v32,0x0c,c[ 8].y);
    lsst_b32(v32,0x10,c[ 4].x);
    lsst_b32(v32,0x14,c[ 4].y);
    lsst_b32(v32,0x18,c[12].x);
    lsst_b32(v32,0x1c,c[12].y);
    lsst_b32(v32,0x20,c[ 1].x);
