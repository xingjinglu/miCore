...
void gfxcoder::sfft5x5_r2c_qerm_s3x3()
{
    uint2 c[24];
    ...
    s_movk_i32(s8,144);
    s_shl_b32(s9,gidy,2);
    s_mul_i32(s10,gidx,s8);
    s_sync(lgkmcnt(0));
    //===========================================================
    //nblocks=(ncells+15)>>4
    //m=ncells-gidx*144
    //n=gidx<nblocks-1?144:m
    //d_r+=by*ldr+bx*144+tid
    //===========================================================
    s_add_u32(s11,gidx,ic(15));
    s_shr_b32(s12,s11,4);
    s_sub_u32(s13,ncells,s10);
    s_sub_u32(s11,s12,ic(1));
    s_cmp_u32(gidx,s11,ge);
    s_cmov_b32(s8,s13);
    s_mul_i32(s11,s9,ldr);
    s_shl2_add_u32(s12,s10,s11);
    S_ADD_U64_U32(rbase,rbase,s12);
    v_shl_b32(v2,tid,2);
    v_cmp_u32(s8,tid,gt);
    ...
    s_andexec_b64(s8,vcc);
    ldg_b32(c[0].x,rbase,v2,0x0);
    s_mov_b64(exec,s8);
    FFT32x32_R2C_CALC_LDS_PTRS(aperm,xsst,xsld,ysst,osst,osld,x,y,p,q,u,v,s7);
    FFT32x32_SET_RF();
    FFT32x32_SET_SCOEF();
    s_sync(vmcnt(0));
    v_cmpk_u32(144,tid,gt);
    s_andexec_b64(s4,vcc);
    lsst_b32(v2,0x0,c[0].x);
    s_mov_b64(exec,s4);
    v_sub_u32(v6,ic(2),x);
    v_mad_u24(v7,ic(9),y,v6);
    v_shl_b32(v2,v7,2);
    s_sync(lgkmcnt(0));
    s_barrier();
    s_mov_b64(s4,exec);
    v_cmpx_u32(ic(3),x,gt);
    lsld_b32(c[0].x,v2,0x18);
    lsld_b32(c[0].y,v2,0x0c);
    lsld_b32(c[1].x,v2,0x00);
    s_mov_b64(exec,s4);
    s_sync(lgkmcnt(0));
    s_barrier();
    VFFT32_C2(c,xsst,xsld);
    HFFT32(c,ysst,ysld,u,aperm,s4,_C2);
    //===========================================================
