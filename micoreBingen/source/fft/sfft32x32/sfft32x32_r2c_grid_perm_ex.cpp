...
/*args : d_c, ldc, bks, d_r, ncells, nx, ldx, ldy, grid, tile, xmagic, xshift, ymagic, yshift, ny*/

void gfxcoder::sfft5x5_r2c_grid_perm_ex()
{
    uint2 c[24];
    ...
    const uint32_t xo     =v37;
    const uint32_t yo     =v38;
    ...
    CALC_SPLIT_IDX(imap,icell_y,icell,grid_x,xmagic,ymagic,s23);
    v_mul_lo_u32(v42,ldx,imap);
    v_mul_u24(xo,tile_x,icell_x);
    v_mul_u24(yo,tile_y,icell_y);
    v_add_u32(xo,xo,x);
    v_mad_u24(v43,nx,yo,v42);
    v_sub_u32(yo,ny,yo);
    v_add_shl_u32(rvo,v43,xo,2);
    S_MUL_U64_U32(s8,s22,ldy);
    S_ADD_U64(rbase,rbase,s8);
    //==================================================================
    FFT32x32_R2C_LDR_SPLIT_EX(c,rbase,rvo,xo,yo,icell,ncells,nx);
