...
void gfxcoder::sfft5x5_r2c_qerm_s5x5()
{
    uint2 c[24];
    ...
    ldg_b32(c[0].x,rbase,v3,0x0);
    s_mov_b64(exec,s8);
    FFT32x32_R2C_CALC_LDS_PTRS(aperm,xsst,xsld,ysst,osst,osld,x,y,p,q,u,v,s7);
    FFT32x32_SET_RF();
    FFT32x32_SET_SCOEF();
    s_sync(vmcnt(0));
    lsst_b32(v3,0x0000,c[0].x);
    v_sub_u32(v6,ic(4),x);
    v_mad_u24(v7,ic(25),y,v6);
    v_shl_b32(v3,v7,2);
    s_sync(lgkmcnt(0));
    s_barrier();
    v_cmp_u32(ic(5),x,gt);
    s_andexec_b64(s4,vcc);
    lsld_b32(c[0].x,v3,0x50);
    lsld_b32(c[0].y,v3,0x3c);
    lsld_b32(c[1].x,v3,0x28);
    lsld_b32(c[1].y,v3,0x14);
    lsld_b32(c[2].x,v3,0x00);
    s_mov_b64(exec,s4);
    s_sync(lgkmcnt(0));
    s_barrier();
