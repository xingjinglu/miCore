...
void gfxcoder::sfft4x4_c2r_g5x5()
{
    uint2 c[12];
    ...
    s_cmov_b32(s8,s9);
    v_mad_u24(v17,ic(25),y,flipx);
    v_shl_b32(v16,v17,2);
    v_cmp_u32(ic(5),flipx,gt);
    s_barrier();
    s_andexec_b64(s0,vcc);
    lsst_b32(v16,0x00,c[0].x);
    lsst_b32(v16,0x14,c[0].y);
    lsst_b32(v16,0x28,c[4].x);
    lsst_b32(v16,0x3c,c[4].y);
    lsst_b32(v16,0x50,c[2].x);
    s_mul_i32(s2,ldr,gidy);
    s_mov_b64(exec,s0);
    s_shl2_add_u32(s3,s12,s2);
    S_ADD_U64_U32(rbase,rbase,s3);
    v_addk_u32(v3,256,tid);
    v_shl_b32(v2,tid,2);
    v_cmp_u32(s0,v3,s8,lt);
    s_sync(lgkmcnt(0));
    s_barrier();
    lsld_b32(v0,v2,0x000);
    lsld_b32(v1,v2,0x400);
    v_cmpx_u32(s8,tid,gt);
    s_sync(lgkmcnt(0));
    stg_b32(rbase,v2,0x000,v0);
    s_mov_b64(exec,s0);
    stg_b32(rbase,v2,0x400,v1);
    s_endp();
}