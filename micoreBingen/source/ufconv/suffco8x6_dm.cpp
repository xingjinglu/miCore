...
void gfxcoder::suffco8x6_dm( uint32_t mask )
{
    ...
    const uint32_t co  =v111;
    const uint32_t asld=v112;
    const uint32_t bsld=v113;
    const uint32_t asst=v114;
    const uint32_t bsst=v115;
    const uint32_t csst=v116;
    const uint32_t csld=v117;
    const uint32_t tidx=v118;
    ...
    s_mul_i32(s18,k,ng);
    s_mul_i32(s19,n,ng);
    UFCO_COMPUTE_START_ADDR_A(ao,m,s18,v3,dimx,amag,v4,s20,1);
    v_shr_b32(v3,v0,2);
    ...
    s_sync(vmcnt(0));
    lsstx_b64x2(asst,0x00,apld+0,0x04,apld+2);
    lsstx_b64x2(asst,0x08,apld+4,0x0c,apld+6);
