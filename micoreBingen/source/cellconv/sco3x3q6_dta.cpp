...
void gfxcoder::sco3x3q6_dta()
{
    ...
    const uint32_t gidy  =s30;
    const uint32_t gidz  =s31;
    ...
    S_ADD_U64_U32(abase+1*2,abase+0*2,s29);
    S_ADD_U64_U32(abase+2*2,abase+1*2,s29);
    S_ADD_U64_U32(abase+3*2,abase+2*2,s29);
    S_ADD_U64_U32(abase+4*2,abase+3*2,s29);
    S_ADD_U64_U32(abase+5*2,abase+4*2,s29);
    S_ADD_U64_U32(abase+6*2,abase+5*2,s29);
    S_ADD_U64_U32(abase+7*2,abase+6*2,s29);
    s_mov_b64(s2,exec);

    for( int i=0; i<8; ++i ){
        v_cmpx_u32(bs,batch,gt);
        ldg_b32(c[i][0],abase+0*2,ao,0x0);
        ldg_b32(c[i][1],abase+1*2,ao,0x0);
        ldg_b32(c[i][2],abase+2*2,ao,0x0);
        ldg_b32(c[i][3],abase+3*2,ao,0x0);
        ldg_b32(c[i][4],abase+4*2,ao,0x0);
        ldg_b32(c[i][5],abase+5*2,ao,0x0);
        ldg_b32(c[i][6],abase+6*2,ao,0x0);
        ldg_b32(c[i][7],abase+7*2,ao,0x0);
        s_mov_b64(exec,s2);
        if(i<7){
